...
model = NeuralNet(28*28, 1000, 2000, num_classes)
...
parametrize.register_parametrization(model.linear1,'weight',linear_layer_parameterization(model.linear1))
parametrize.register_parametrization(model.linear2,'weight',linear_layer_parameterization(model.linear2))
parametrize.register_parametrization(model.linear3,'weight',linear_layer_parameterization(model.linear3))
...
# Seperating the Digit 9 from Original MNIST Training Data Set
required_index = train_dataset.targets == 9

train_dataset.data = train_dataset.data[required_index]

train_dataset.targets = train_dataset.targets[required_index]

train_loader_9 = DataLoader(dataset=train_dataset,batch_size=batch_size,shuffle=True,drop_last=True)
...
# Calling the Training Function
# Here, We are Fine Tuning the Digit 9 
Training(model,train_loader_9,learning_rate,num_epochs,Total_num_steps=30)
# Calling the Test Function after Fine Tuning the Digit 9
Testing(model,test_loader)
